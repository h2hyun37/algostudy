...
public class Problem11 {
	 ...
	 * 생각나는 문제 풀이 최적화? 방법
	 * 
	 * 1. 일단 4자리 수 중 0 이 있으면 skip 
	 * -> 연산 횟수를 줄일 수 있..나?
	 * 
	 * 2. 한칸씩 이동하면서 계산? ex> 1 2 3 4 5 있을때에 , 1*2*3*4 구하고 난 다음에는 (1*2*3*4) / 1 * 5 
	 * -> 효과가 있으려나...
	 * 
	 * 3. 
	 * 
	...
	public static void main(String[] args) {
		...
		// 2차원 배열의 가로,세로 길이. 가로 길이는 처음 배열의 길이가 나머지와 모두 같다고 가정 
		int matrixWidth 	= p11.strMatrix[0].length;
		int matrixHeight	= p11.strMatrix.length;
		
		// 연속으로 곱할 숫자 갯수
		int multiplyNumCount = 4;
		...
		for (int verticalIdx = 0; verticalIdx < matrixHeight; verticalIdx++) {
			for (int horizontalIdx = 0; horizontalIdx < matrixWidth; horizontalIdx++) { 

				
				/* 
				 * 가로, 세로, 대각선1 (좌상우하) , 대각선2 (우상좌하) 의 곱을 저장할 변수들 
				 */
				int multiplyHorizonal = 1;
				int multiplyVertical = 1;
				int multiplyDiagonal1 = 1;
				int multiplyDiagonal2 = 1;
				
				/*
				 *  가로 계산. 
				 *  단 가로 계산할 숫자 범위가 가로폭을 넘지 않아야 함 
				 */
				if (horizontalIdx + multiplyNumCount <= matrixWidth) {
					
					for (int i=0; i < multiplyNumCount; i++) { 
						int num = Integer.parseInt(p11.strMatrix[verticalIdx][horizontalIdx+i]);
						
						// 숫자가 0인 경우는 계산하지 말고 pass 
						if (num == 0) {
							multiplyHorizonal = 0;
							break;
						}
						
						multiplyHorizonal *= num;
					}
				...
				/*
				 * 세로 계산 
				 * 단 세로 계산할 숫자 범위가 세로폭을 넘지 않아야 함 
				 */				
				if (verticalIdx + multiplyNumCount <= matrixHeight) {
					
					for (int i=0; i < multiplyNumCount; i++) { 
						int num = Integer.parseInt(p11.strMatrix[verticalIdx+i][horizontalIdx]);
				...
				/*
				 * 대각선1 계산 
				 * 단 계산할 숫자 범위가 가로, 세로폭을 넘지 않아야 함
				 */
				if (horizontalIdx + multiplyNumCount <= matrixWidth
						&& verticalIdx + multiplyNumCount <= matrixHeight) {

					for (int i = 0; i < multiplyNumCount; i++) {
						int num = Integer.parseInt(p11.strMatrix[verticalIdx
								+ i][horizontalIdx + i]);
				 ...
				 * 단 계산할 숫자 범위가 가로, 세로폭을 넘지 않아야 함
				 * 
				 * 대각선2는 우상좌하 이므로 가로 폭 계산시 유의하여야 한다
				 */
				if (horizontalIdx - multiplyNumCount + 1 >= 0
						&& verticalIdx + multiplyNumCount <= matrixHeight) {
				 ...
				 * 가장 큰 수를 비교하여 저장
				 */
				int biggerNum1 = multiplyHorizonal > multiplyVertical ? multiplyHorizonal
						: multiplyVertical;
		...
		System.out.println("전체 실행 시간 : " + (totalTimeFinish - totalTimeStart)
				/ 1000.0);
